/*
Deployment script for Template_1
...
GO
:setvar DatabaseName "Template_1"
:setvar DefaultFilePrefix "Template_1"
...
IF EXISTS (select top 1 1 from [dbo].[Evroliga])
...
IF EXISTS (select top 1 1 from [dbo].[Hala])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[Igrac])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[IgracIgra])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[Klub])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[Nalog])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[Rezervacija])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[Sudija])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[Trener])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Igrac])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Igrac] ([LICBR_IGR], [IME_IGR], [PRZ_IGR], [PDATUG_IGR], [KDATUG_IGR], [POZ_IGR], [VIS_IGR], [Klub_ID_KLB])
        SELECT   [LICBR_IGR],
                 [IME_IGR],
                 [PRZ_IGR],
                 [PDATUG_IGR],
...
CREATE TABLE [dbo].[tmp_ms_xx_IgracIgra] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_IgracIgra_PK1] PRIMARY KEY CLUSTERED ([Utakmica_OZN_UTK] ASC, [Igrac_LICBR_IGR] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[IgracIgra])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_IgracIgra] ([Utakmica_OZN_UTK], [Igrac_LICBR_IGR])
        SELECT   [Utakmica_OZN_UTK],
                 [Igrac_LICBR_IGR]
        FROM     [dbo].[IgracIgra]
        ORDER BY [Utakmica_OZN_UTK] ASC, [Igrac_LICBR_IGR] ASC;
    END
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Klub])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Klub] ([ID_KLB], [NAZ_KLB], [DRZ_KLB], [GRD_KLB], [Evroliga_OZN_LIG], [POB_KLB], [POR_KLB], [KOSRAZ_KLB])
        SELECT   [ID_KLB],
                 [NAZ_KLB],
                 [DRZ_KLB],
                 [GRD_KLB],
...
CREATE TABLE [dbo].[tmp_ms_xx_Sudi] (
    ...
    [Utakmica_OZN_UTK] VARCHAR (64) NOT NULL,
    CONSTRAINT [tmp_ms_xx_constraint_Sudi_PK1] PRIMARY KEY CLUSTERED ([Utakmica_OZN_UTK] ASC, [Sudija_LICBR_SUD] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Sudi])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Sudi] ([Utakmica_OZN_UTK], [Sudija_LICBR_SUD])
        SELECT   [Utakmica_OZN_UTK],
                 [Sudija_LICBR_SUD]
        FROM     [dbo].[Sudi]
        ORDER BY [Utakmica_OZN_UTK] ASC, [Sudija_LICBR_SUD] ASC;
    END
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Sudija])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Sudija] ([LICBR_SUD], [DRZ_SUD], [IME_SUD], [PRZ_SUD], [Evroliga_OZN_LIG])
        SELECT   [LICBR_SUD],
                 [DRZ_SUD],
                 [IME_SUD],
                 [PRZ_SUD],
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Utakmica])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Utakmica] ([OZN_UTK], [Evroliga_OZN_LIG], [RBRKOLA_UTK], [DOMPOENI_UTK], [GOSTPOENI_UTK], [Klub_ID_KLB], [Klub_ID_KLB1])
        SELECT   [OZN_UTK],
                 [Evroliga_OZN_LIG],
                 [RBRKOLA_UTK],
                 [DOMPOENI_UTK],
...
GO
PRINT N'Altering [dbo].[DateValidationReservation]...';


GO
ALTER PROCEDURE DateValidationReservation
	(
	@startDate date,
	@endDate date,
	@hallId varchar(15),
